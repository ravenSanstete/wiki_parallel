...
public class ConsumerTool extends Thread implements MessageListener, ExceptionListener {
	...
	private AlphaCounter word_counter;

	public ConsumerTool(AlphaCounter c){
	    word_counter = c;
    }
    ...
    public void onMessage(Message message) {
        try {
            if (message instanceof TextMessage) {
                String line = ((TextMessage) message).getText();
                if(line.equals(Util.termination)){
                    Thread.currentThread().interrupt();
                }

                // do the critical task here
                Words s = Util.morph(line);
                for(String w: s){
                    word_counter.count(w);
                    word_num += 1;
                    //producer.send(session.createTextMessage(w));
                }
